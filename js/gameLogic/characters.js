class CharacterManager {
    ...
    initializeCursor(characterName = null) {
        // Initialize cursor object if it doesn't exist
        if (!this.cursor) {
            this.cursor = {
                x: canvas.width / 2,
                y: canvas.height - 64,
        ...
        // Load cursor image if not already set
        if (!this.cursor.image && this.currentCharacter) {
            this.cursor.image = this.getCursorImage(this.currentCharacter.name);
        }
        ...
        console.log("Cursor initialized:", {
            ...
            image: this.cursor.image,
            position: { x: this.cursor.x, y: this.cursor.y }
        });
    }
    
    getCursorImage(characterName) {
        // Implement your logic to get the cursor image
        // Example:
        return window.assetLoader.getImage(`character${characterName}Cursor`);
    }
